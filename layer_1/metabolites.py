        ...
        if name not in self.df.index :
            ...
            for meta in self.__class_model_instance.Stoichio_matrix.index :
                ...
                if meta not in self.df.index :
                    self.__class_model_instance.Stoichio_matrix.drop(meta, axis=0, inplace=True)

            # And from every mention of it in the reaction dataframe
            for reaction in self.__class_model_instance.reactions.df.index :
                key_to_remove = [cle for cle in self.__class_model_instance.reactions.df.loc[reaction, 'Metabolites'].keys() if name in cle]
                for key_to_remove in key_to_remove:
                    self.__class_model_instance.reactions.df.loc[reaction, 'Metabolites'].pop(key_to_remove)
