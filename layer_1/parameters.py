        ...
        for meta in self.__class_model_instance.metabolites.df.index :
            ...
            if self.__class_model_instance.metabolites.df.loc[meta, 'External'] == True :
                ...
                if (meta+"_para") not in self.df.index :
                    ...
                    self.__class_model_instance.elasticity.p[meta+"_para"] = [0 for i in range(self.__class_model_instance.elasticity.p.shape[0])]

                    for reaction in self.__class_model_instance.Stoichio_matrix.columns :
                        if self.__class_model_instance.Stoichio_matrix.loc[meta, reaction] != 0 :
                            self.__class_model_instance.elasticity.p.loc[reaction, meta+"_para"] =  -0.5*self.__class_model_instance.Stoichio_matrix.loc[meta, reaction]